...
public class FlixTriangle: FlixObject {
  ...
  public init(triangle: Triangle, startingBody: PHYRigidBody, color: Color, size: Vector3 = .one) {
    ...
    self.color = color
    self.rigidbody = PHYRigidBodyFromRaylibModel(model: model!, scale: scale, isStatic: false, mass: 0.01, collisionType: .concave)
    ...
    rigidbody!.linearVelocity = startingBody.linearVelocity.vector3.scale(0.5).phyVector3
